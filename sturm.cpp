 ...
 */
{
    ...
    for (auto i = order; i >= 0; --i) coefs.push_back(sturm_seq[0].coef[i]);
    seq.push_back(coefs);
    for (auto i = 0; i <= num_poly; ++i) {
        coefs.clear();
        for (auto j = sturm_seq[i].ord; j >= 0; --j) coefs.push_back(sturm_seq[i].coef[j]);
        seq.push_back(coefs);
    }
    return seq;
}
...
void Sturm::show_sturm_sequence(const std::vector<std::vector<double>>& seq) {
    ...
    for (const auto& poly : seq) {
        for (const auto& coef : poly) std::cout << coef << ' ';
        if (first) {
            std::cout << "\n";
            first = false;
        }
 ...
 */
{
    ...
    for (auto i = 1; i <= order; i++) *fp++ = *fc++ * i / f;
...
std::vector<double> Sturm::get_real_roots() {
    ...
    for (auto i = 0; i != nroots; ++i) roots_vec.push_back(roots[i]);
 ...
 */
{
    ...
    if ((nroot = atmin - atmax) == 1) {
        ...
        if (mod_rf(sturm_seq->ord, sturm_seq->coef, min, max, &roots[0])) return;
        ...
        if (its == max_it) {
            std::cerr << "bisect: overflow min " << min << " max " << max << " diff " << max - min
                      << " nroot " << nroot << " n1 " << n1 << " n2 " << n2 << "\n";
            roots[0] = mid;
        }
        return;
    }
...
void Sturm::set_coeffs(int ord, const std::vector<double>& coefs) {
    ...
    for (int i = ord + 1; i < max_order; ++i) sturm_seq[0].coef[i] = 0.0;
}
